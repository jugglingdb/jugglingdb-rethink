                                ...
                                r.db(_this.database).table(model).indexList().run(client, function(error, cursor) {
                                    ...
                                    cursor.toArray(function(error, list) {
                                        ...
                                        async.each(Object.keys(properties), function (property, cb3) {
                                            if (properties[property].index && list.indexOf(property) < 0) {
                            ...
                            if (list.indexOf(model) < 0) {
                                ...
                                r.db(_this.database).table(model).indexList().run(client, function(error, cursor) {
                                    ...
                                    cursor.toArray(function(error, list) {
                                        ...
                                        Object.keys(_this._models[model].properties).forEach(function (property) {
                                            if (_this._models[model].properties[property].index && list.indexOf(property) < 0)
                                                actual = false;
                                        });
                                        cb2();
                                    });
    ...
    _this.pool.acquire(function(error, client) {
        ...
        if (filter.where) {
            Object.keys(filter.where).forEach(function (k) {
                ...
                switch (spec) {
                    case false:
                        if(!indexed && _this._models[model].properties[k].index) {
